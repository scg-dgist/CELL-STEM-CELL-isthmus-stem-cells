! ---------------------------------------------------------------------------- !
!     Lateral clonal dynamics including barrier segments                       !
!     David Jorg                                                               !
!																																							 !
!     compile:																																 !
!				gfortran -cpp -Dsingle llr_barrier.f90 -ffree-line-length-none         !
!       -fbackslash -Ofast -o llr_barrier                                      !
! ---------------------------------------------------------------------------- !
...
program clone
  ...
  ! computation variables
	...
	real(kind=8),     dimension(:),         allocatable   :: pp_field, pp_out, pp_in
	integer                                               :: update_field, expanding_field, gland_size, unlabelled_gland, num_clones, num_labelled_clones, barrier_number, dist
  integer,          dimension(:),         allocatable   :: relative_clone_sizes, global_unlabelled_glands, global_monoclonal_glands, global_num_clones, global_num_labelled_clones, barrier_angles, barrier_list
  ...
  read(cl_args(8),*) num_colors
	...
	allocate(pp_field(1:num_segments_2), pp_in(1:num_segments_2), pp_out(1:num_segments_2), barrier_list(1:num_segments))

	allocate(relative_clone_sizes(0:bins_clones), barrier_angles(1:num_segments**2))
	allocate(global_relative_clone_sizes(0:bins_clones,0:readout_points), global_unlabelled_glands(0:readout_points), global_monoclonal_glands(0:readout_points), global_num_clones(0:readout_points), global_num_labelled_clones(0:readout_points), global_barrier_number_dist(0:num_segments,0:readout_points), global_barrier_angle_dist(2:num_segments,0:bins_angles,0:readout_points))
  ...
  neighbors(num_segments_2,N_R) = 1
	...
	do r=1,num_runs
		...
		do while (lrp<=readout_points)
			...
			pp_in = 0.
			pp_out = 0.
			! -- initialize partial propensities for barrier segments
			do i=2,num_segments_2,2
				pp_field(i) = 0.

				if (pres(i)==1) then
					! if already present, the only channel is disappearance
					pp_in(i) = 0.
					pp_out(i) = omega_out
				else
					! if not present, determine apperance rate
					pp_in(i) = omega_in
					do j=1,barrier_number
						jc = barrier_list(j)

						call get_barrier_distance(mod2, rmod_numseg2, gland_size, barrier_equivalent, num_segments_2, pres, i, jc, dist)

						pp_in(i) = pp_in(i) * potential(dist)
					end do
			...
			! total propensity
			tp = sum(pp_field) + sum(pp_out) + sum(pp_in)
			...
			! - barrier site disappearing
			if (searching) then
				do i=2,num_segments_2,2
					p_sum = p_sum + pp_out(i)
					if (reaction<p_sum.and.searching) then
						pres(i) = 0
						searching = .false.
						exit
			...
			! - barrier site appearing
			if (searching) then
				do i=2,num_segments_2,2
					p_sum = p_sum + pp_in(i)
					if (reaction<p_sum.and.searching) then
						pres(i) = 1
						! barrier segment takes the color label from a random neighbor
						call random_number(die)
						field(i) = field(neighbors(i, ceiling2(2. * die)))
						searching = .false.
						exit
					end if
				end do
			...
			if (rp>lrp.and.equilibrated) then
				...
				do k=lrp+1,rp
					...
					if ((num_clones==1).and.(unlabelled_gland==0)) then
						global_monoclonal_glands(k) = global_monoclonal_glands(k) + 1
					end if
				end do
  ...
  open(unit=1,file=trim(directory) // 'global_relative_clone_sizes.csv')
	...
	open(unit=1,file=trim(directory) // 'global_monoclonal_glands.csv')
		do k=0,readout_points
			write(1,'(i20,a)') global_monoclonal_glands(k)
		end do
	close(1)
